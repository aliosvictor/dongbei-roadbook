...
def generated_variables(itinerary: dict, snapshot: dict) -> str:
    ...
    values["primary-extra-time"] = f"{delta_minutes // 60} 小时 {delta_minutes % 60:02d} 分"
...
def main() -> None:
    ...
    if args.from_snapshot:
        ...
        for key, spec in itinerary["amap_route_specs"].items():
            validate_snapshot(spec, snapshot["routes"][key], places)
        LINKS.write_text(generated_links(itinerary, snapshot), encoding="utf-8")
        VARIABLES.write_text(generated_variables(itinerary, snapshot), encoding="utf-8")
