...
- `data/itinerary.json` 是路线节点、每日线路和蓝点拍摄位置的统一结构化基准。
- `index.qmd` 是三套路线的选择规则与总览；`primary.qmd`、`option-skip-qiqian.qmd`、`option-reverse.qmd` 分别承载各自的逐日安排。
...
- 地图由 `scripts/build_maps.py` 在本地生成；路线线条连接 `data/itinerary.json` 中的控制点，蓝点对应同一数据文件中的重点拍摄位置。实际行驶使用实时导航、交警信息和现场标识。
