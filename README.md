...
这是 2026-09-26 至 2026-10-03 的 Quarto 在线路书。备用方案从齐齐哈尔出发，经蘑阿公路、阿尔山、莫日格勒河、卡线、根河和五大连池，于 10 月 3 日晚抵达哈尔滨；只有选择奇乾晨景且道路、住宿与天气条件全部成立时，才执行主方案。
...
- `sources.qmd`：核验与准备，分为交通管制与证件、地点与预约、天气与拍摄、无人机、摄影器材、物资清单六部分；有效来源按主题归档。
...
只变更部分路线时，可用 `python3 scripts/update_amap_routes.py --only p03_direct d03_direct` 仅更新指定规格，保留其他快照与核验日期；`--from-snapshot` 仅离线重建指标和链接。地图按实际绘制路段标注日期，总览可显示日期范围。路线专用指标使用 `route-<路线键>-km/time/checked`，避免与 `d03` 等每日地图同名指标混淆。默认 9 月 28 日路线不进河湾，旧河湾路线仅作准入确认后的候选，不能加入默认总里程或绘制为已确认支路。

路线校验同时检查实际经过顺序、起终点和坐标版本，绘图保留高德原始分段边界。地点角色按当天用途设置；莫尔道嘎在主方案为选停补给，在备用方案为住宿。摄影等级不决定点位颜色。修改路线数据或绘图脚本后重建全部地图；`figures/maps/manifest.json` 记录输入及输出校验值，漏生成时发布检查会失败。修改文字后重建字体；不要手改生成的 HTML、地图、指标或链接。

乌苏浪子湖渔村宾馆使用统一名称搜索链接，湖景蓝点不能充当宾馆停车坐标；未核准的晨拍进出不画线、不计入高德里程。10 月 3 日默认使用温泊路线，黑龙山为管理方确认当日开放后的独立条件路线。实心蓝点为计划主拍，紫色空心点为选停补给、短拍或条件备选，橄榄色编号为住宿/换乘，灰色标记仅作方向与里程参照，哈尔滨执行终点须落实到订单门店。
